...
parser.add_argument('--batch_size', type=int, default=100,
        ...
        'data processing.')
parser.add_argument('--corruption_file', type=str, default='tiny_imagenet_c_bar.csv',
...
def main():
    ...
    out_dir = os.path.join(args.out_dir, 'test_corruption')
    ...
    for name, severities in corruptions.items():
        corruption_dir = f"{out_dir}-{name}"
        for i, severity in enumerate(severities):
            severity_dir = f"{corruption_dir}-{'{:.2f}'.format(severity)}"
